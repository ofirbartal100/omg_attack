...
class BirdsViewMaker(TrafficViewMaker):
    ...
    def objective(self, emb_dict):
        ...
        o,_ = self.model.birds_model(self.normalize(emb_dict["originals"]))
...
class MNISTViewMaker(BirdsViewMaker):
    ...
    def objective(self, emb_dict):
        ...
        o,_ = self.model.mnist_model(self.normalize(emb_dict["originals"]))
        o_prob = F.softmax(o, dim = -1)
        o = o_prob.argmax(1, keepdim = True)

        v1,_ = self.model.mnist_model(emb_dict["views1"])
        v1_prob = F.softmax(v1, dim = -1)
        v1 = v1_prob.argmax(1, keepdim = True)

        v2,_ = self.model.mnist_model(emb_dict["views2"])
        v2_prob = F.softmax(v2, dim = -1)
        v2 = v2_prob.argmax(1, keepdim = True)
    ...
    def wandb_logging(self, emb_dict):
        with torch.no_grad():
            ...
            if self.global_step % self.logging_steps == 0:
                ...
                img_border = torch.cat([ MNISTViewMaker.paint_border(img[c],confused_orig[c],pad=2).unsqueeze(0)  for c in range(self.disp_amnt)],dim=0)
                unnormalized_view1_border = torch.cat([ MNISTViewMaker.paint_border(unnormalized_view1[c],confused_v1[c],pad=2).unsqueeze(0)  for c in range(self.disp_amnt)],dim=0)
                unnormalized_view2_border = torch.cat([ MNISTViewMaker.paint_border(unnormalized_view2[c],confused_v2[c],pad=2).unsqueeze(0)  for c in range(self.disp_amnt)],dim=0)
...
class CIFARViewMaker(BirdsViewMaker):
    ...
    def objective(self, emb_dict):
        ...
        o,_ = self.model.cifar_model(self.normalize(emb_dict["originals"]))
        o_prob = F.softmax(o, dim = -1)
        o = o_prob.argmax(1, keepdim = True)

        v1,_ = self.model.cifar_model(emb_dict["views1"])
        v1_prob = F.softmax(v1, dim = -1)
        v1 = v1_prob.argmax(1, keepdim = True)

        v2,_ = self.model.cifar_model(emb_dict["views2"])
        v2_prob = F.softmax(v2, dim = -1)
        v2 = v2_prob.argmax(1, keepdim = True)
    ...
    def wandb_logging(self, emb_dict):
        with torch.no_grad():
            ...
            if self.global_step % self.logging_steps == 0:
                ...
                img_border = torch.cat([ BirdsViewMaker.paint_border(img[c],confused_orig[c],pad=1).unsqueeze(0)  for c in range(self.disp_amnt)],dim=0)
                unnormalized_view1_border = torch.cat([ BirdsViewMaker.paint_border(unnormalized_view1[c],confused_v1[c],pad=1).unsqueeze(0)  for c in range(self.disp_amnt)],dim=0)
                unnormalized_view2_border = torch.cat([ BirdsViewMaker.paint_border(unnormalized_view2[c],confused_v2[c],pad=1).unsqueeze(0)  for c in range(self.disp_amnt)],dim=0)
    ...
    def training_step_end(self, train_step_outputs):
        ...
        return {'loss':loss ,'gen_acc':metrics['gen_acc'],'encoder_acc':metrics['encoder_acc']}
