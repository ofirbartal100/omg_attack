...
class CifarModel(BaseModel):
    ...
    def __init__(self, cifar_model_path, vgg_config_path ,input_specs: List[Dict[str, Any]]):
        ...
        self.cifar_model = models.resnet34()
        IN_FEATURES = self.cifar_model.fc.in_features 
        print(f'IN_FEATURES {IN_FEATURES}')
        fc = torch.nn.Linear(IN_FEATURES, 10)
        if '80' in cifar_model_path:
            fc = torch.nn.Linear(IN_FEATURES, 8)
        self.cifar_model.fc = fc

        checkpoint_f = torch.load(self.cifar_model_path, map_location='cpu')
        self.cifar_model.load_state_dict(checkpoint_f["state_dict"])
